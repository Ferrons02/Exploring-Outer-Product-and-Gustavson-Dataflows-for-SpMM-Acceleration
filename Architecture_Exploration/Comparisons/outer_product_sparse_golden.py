...
def outer_product_sparse_golden(reuse_policy, X_elem_size, Y_elem_size, X_sparsity, X_rows, X_columns, Y_rows, Y_columns):
    # Matrix generation
            ...
            for k in range(X_column_iters):
                ...
                for X_column_block in range(X_column_blocks):
                    ...
                    for Y_row_block in range(Y_row_blocks):
                        ...
                        out_buffer = [Z[j - X_rows * (k + starting_column)][start_Y:end_Y] if j is not None else [
                                                                                                                     '0' * Out_data_size] * (
                                                                                                                             end_Y - start_Y)
            ...
            for meta_chunk in range(meta_iters):
                ...
                for j in range(ending_column - starting_column + 1):
                    list_of_column_elem.append([x for x in X_meta_positions if
                                                (starting_column + j) * X_rows <= x < (
                                                            starting_column + j + 1) * X_rows])
